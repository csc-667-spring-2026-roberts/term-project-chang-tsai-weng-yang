(() => {
  ...
  function init() {
    ...
    if (gameRoomCloseBtn) gameRoomCloseBtn.addEventListener("click", () => closeModal());
    if (gameRoomBackdrop) {
  ...
  function switchTab(tabName) {
    panelJoin && panelJoin.classList.remove("active");
    panelCreate && panelCreate.classList.remove("active");
    panelWaiting && panelWaiting.classList.remove("active");
    tabJoin && tabJoin.classList.remove("active");
    tabCreate && tabCreate.classList.remove("active");
    tabWaiting && tabWaiting.classList.remove("active");
    if (tabName === "join") {
      panelJoin && panelJoin.classList.add("active");
      tabJoin && tabJoin.classList.add("active");
    } else if (tabName === "create") {
      panelCreate && panelCreate.classList.add("active");
      tabCreate && tabCreate.classList.add("active");
    } else if (tabName === "waiting") {
      panelWaiting && panelWaiting.classList.add("active");
      tabWaiting && tabWaiting.classList.add("active");
    }
  }

  async function joinRoom() {
    const roomId = inputRoomId && inputRoomId.value && inputRoomId.value.trim().toUpperCase();
    if (!roomId) { showError(joinError, "Please enter a room ID"); return; }
    if (roomId.length !== 6) { showError(joinError, "Room ID must be 6 characters"); return; }
    await getCurrentUserId();
    if (!currentUserId) { showError(joinError, "Authentication required"); return; }
  ...
  async function createRoom() {
    await getCurrentUserId();
    if (!currentUserId) { showError(createError, "Authentication required"); return; }
  ...
  function showError(element, message) { if (!element) return; element.textContent = message; element.hidden = false; }
  function clearError(element) { if (!element) return; element.hidden = true; element.textContent = ""; }
  function showLoading(element) { if (!element) return; element.hidden = false; }
  function hideLoading(element) { if (!element) return; element.hidden = true; }
